...
{
    char message[64];
    int n = snprintf(message, sizeof(message), "%d\n", v);
    return send_all(sid, message, n);
}
...
void * thread_main(void * arg_in)
{
    ...
    int guess;
    int result;
    rv = send_int(sockfd, gmn_get_max());
    if (rv<0) thread_die(arg);

    while (1) {
        rv = recv_lines(sockfd, message, MSG_BUF_SIZE);
        if (rv<0) thread_die(arg);
        if (sscanf(message, "%d", &guess) != 1) thread_die(arg); //what i was missing

        result = gmn_check(&gmn, guess);

        rv = send_int(sockfd, result);
        if (rv<0) thread_die(arg);

        if (result == 0) {
            rv = send_all(sockfd, gmn_get_message(&gmn), strlen(gmn_get_message(&gmn)));
            if ( rv< 0) thread_die(arg);
            break;
        }
    }
