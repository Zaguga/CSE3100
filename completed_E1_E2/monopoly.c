...
//Otherwise, return 0
int transaction(TPlayer *p1, TPlayer *p2, int amount) //PASSED BY REFERENCE -- use --> pointer way
{
	long bal1 = p1 -> balance;
	//long bal2 = p2 -> balance; dont actually use

	if (bal1 - (long)amount >= 0){
		p1 -> balance -= amount;
		p2 -> balance += amount;
...
int one_round(int m, int n, TPlayer p[], TProperty prop[]) //PASSED BY VALUE -- use . the "object" way
{
	for(int i = 0; i < m; i++) // i = player id of current turn
	{
		int steps = rand() % 6 + 1 + rand() % 6 + 1;
		//fill in the code below

		p[i].loc = (p[i].loc + steps) % n; //increase location, mod by n to keep in bounds
		int idx = p[i].loc; //for easy use later

		if (prop[idx].owner_id == -1){ //if noone owns it
			int id = p[i].id; //get their id
			prop[idx].owner_id = id; //set their id as owner
			//p[i].balance -= prop[idx].rent; //pay rent
		}
		else{
			if (prop[idx].owner_id != p[i].id){
				int owner = prop[idx].owner_id;
				if (transaction(&p[i], &p[owner], prop[idx].rent) == 0){
					return 0;//stops round
				}
			}
		}
...
// gcc -std=c11 -Wall -Wextra -Werror monopoly.c -o monopoly
