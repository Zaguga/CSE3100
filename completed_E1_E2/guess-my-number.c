...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int guess;
    int result;
    while(read(fdp[0], &guess, sizeof(guess)) > 0){
        result = gmn_check(&gmn, guess); //&gmn fix
    ...
    write(fdc[1], gmn_get_message(&gmn), strlen(gmn_get_message(&gmn)) + 1);
...
int main(int argc, char *argv[])
{
    ...
    close(fdp[0]); //the parent doesn't read from the parent
    close(fdc[1]); //the parent doesn't write from the child
    if (read(fdc[0], &max, sizeof(max)) != sizeof(max)){
        die("failed to get max from child");
    }
    do { 
        guess = (min + max)/2;
        printf("My guess: %d\n", guess);
        ...
        if(write(fdp[1], &guess, sizeof(guess)) != sizeof(guess)){
            die("failed to send guess to child");
        }
        //     wait for the result from the child -- just a read
        if(read(fdc[0], &result, sizeof(result)) != sizeof(result)){
            die("failed to wait for the result from child");
        }
        if (result > 0)
            min = guess + 1;
        else if (result < 0)
    ...
    int n = read(fdc[0], final_message, sizeof(final_message) - 1);
    if (n>0){
