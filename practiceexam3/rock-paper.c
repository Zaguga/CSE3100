...
void * thread_player(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++ ) {
        ...
        choice->value = r;
        //signal ref that guess is ready in choice value
        choice->status = S_READY;
        //^ only line missing, all others i got straight off the dome
        pthread_cond_signal(&choice->cond); //question then is why did we signal this too
...
/////////////////
        ...
        pthread_barrier_wait(&result->barrier); //idk what this is doing
        // TODO end
