...
void * thread_c(void * arg_in)
{
    ...
    //      call gmn_check() 
        if_correct = gmn_check(&gmn, arg->guess);
        arg->result = if_correct;
...
void * thread_p(void *arg_in)
{
    ...
    while(arg->status != S_MAX){
        pthread_cond_wait(&arg->cond_result, &arg->mutex); //result or guess?
    }
    max = arg->max;
    pthread_mutex_unlock(&arg->mutex);
...
int main(int argc, char *argv[])
{
    ...
    for (unsigned int i = 0; i < NUM_THREADS; i++){
        if (i == 0){
            if(pthread_create(&threads[i], NULL, thread_c, &arg) !=0){
                perror("pthread_create");
                exit(EXIT_FAILURE);
            }
        }
        else{
            if(pthread_create(&threads[i], NULL, thread_p, &arg) !=0){
                perror("pthread_create");
                exit(EXIT_FAILURE);
            }
        }
