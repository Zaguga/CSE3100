...
void *counter(void *t) {
  ...
  int done = 0;
  pthread_mutex_lock(&pdata->mutex); //ONE
  while (!done) {
    pthread_cond_wait(&pdata->cond[id],&pdata->mutex); //THREE
    ...
    pthread_mutex_unlock(&pdata->mutex); //TWO
...
void *tosser(void *t) {
  ...
  while (!done) {
    pthread_cond_wait(&pdata->cond_toss, &pdata->mutex); //THREE
    int c1 = rand_r(&s) & 1;
    int c2 = rand_r(&s) & 1;
    int c3 = rand_r(&s) & 1;
    int c4 = rand_r(&s) & 1;
    ...
    pdata->data = c;
    pdata->ready = 1;
    pthread_cond_signal(&pdata->cond[id]); //FOUR
    pdata->n = pdata->n - 1;
    pthread_mutex_unlock(&pdata->mutex); //TWO
    if (pdata->n == 0){
      done = 1;
