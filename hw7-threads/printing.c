...
void * printer_main(void * arg)
{
    ...
    job_queue_t *jq = printer->jq; //MISSED
    unsigned int id = printer->id;
    pthread_t thread_id = printer->thread_id;
    int job_time; //MISSED
    printer->njobs = 0;
    ...
    while(1)
    {
        //split work -- mutex here
        pthread_mutex_lock(&jq->mutex); //jq pointer
        if (q_num_jobs(jq) > 0){
            job_time = q_fetch_job(jq, printer->id);
...
// TODO: 
    ...
    //      prepare arguments to threads: int id; 
    //                                    job_queue_t * jq; 
    //                                    int njobs;   // number of jobs printed
    //                                    pthread_t thread_id; // thread id
    for (unsigned int i = 0; i < num_printers; i++){
        ...
        printers[i].njobs = 0; //each printer starts with 0 jobs
        // create threads
        if(pthread_create(&printers[i].thread_id, NULL, printer_main, &printers[i]) != 0){
    ...
    //      desctroy mutex.
    pthread_mutex_destroy(&job_queue.mutex); //also unsure
